...
void YOLOV7_face::nms(vector<Yolov5Face_BoxStruct>& input_boxes)
{
...
vector<Yolov5Face_BoxStruct> YOLOV7_face::detect(Mat& frame)
{
	Mat dstimg;
	...
	for (n = 0; n < this->num_proposal; n++)   ///特征图尺度
	{
		...
		if (box_score > this->confThreshold)
		{
			...
			if (class_socre > this->confThreshold)
			{
				...
				k = 0;
				int x = int((pdata[6 + k] - pad_info[0]) / pad_info[2]);
				int y = int((pdata[6 + k + 1] - pad_info[1]) / pad_info[2]);
				float score = pdata[6 + k + 2];
				// PointInfo kpt1 = { Point(x,y), score };
				Point kpt1 =  Point(x,y);
				k += 3;

				x = int((pdata[6 + k] - pad_info[0]) / pad_info[2]);
				y = int((pdata[6 + k + 1] - pad_info[1]) / pad_info[2]);
				score = pdata[6 + k + 2];
				...
				std::vector<cv::Point> kkk={kpt1,kpt2,kpt3,kpt4,kpt5};
...
vector<Yolov5Face_BoxStruct> YOLOV7_face::detect_v8(Mat& frame)
{
	Mat dstimg;
	std::vector<float> pad_info = LetterboxImage(frame, dstimg, cv::Size(this->inpWidth, this->inpWidth));
	...
	nout = pred_dims.at(1);
	// [1,3,640,640][1,15,8400]4+1+5*2               有score[1,20,8400] 4+1+5*3
