...
#include "post/inferModel/customModelSim.h"

#include "app_postprocess.h"
#include "license.h"
#include "cdsServer.h"
#include "httpGetImage.h"
#include "httpMechanical.h"



using namespace std;

BlockingQueue<std::map<std::string, cv::Mat>> g_images(16); // 30 32
BlockingQueue<std::map<std::string, cv::Mat>> g_images2(16);
BlockingQueue<std::map<std::string, cv::Mat>> g_preview_images(7);
// BlockingQueue<std::map<std::string, cv::Mat>> g_capture_images(1);
std::map<std::string, cv::Mat> camImage;
std::mutex mtx_camImage;
    ...
    while (!stop)
    {
      try
      {
        ...
        while (!stop)
        {
          ...
          if (alg_name == "student_absent_detection" || alg_name == "leaving_post" || alg_name == "sleeping_post"  || alg_name == "cabinet_indicator_light_detection" || alg_name == "cv_asset_deficiency_detection")
          ...
          else if (alg_name == "wash_hands_detection")
            postprocess.WashHandProcess(result, cam_config, cam_config_open);
          else
...
public:
  ...
  void run()
  {
    ...
    else if (algName == "garbage_exposure" || algName == "cv_large_item_inspection" || algName == "cv_aisle_cargo_detection"  || algName == "phone_detection" || algName == "play_phone_detection")
    {
    ...
    while (1)
    {
      ...
      for (const auto &pair : _rtsp_image5)
      {
        ...
        // 资产识别，冷库还是单ROI，冷库检测叉车就行
        // if(algName=="cv_machine_room_property_detection" or algName=="cv_cold_storage_goods_detection"){
        if(algName=="cv_machine_room_property_detection" ){
          json cam_config;
          for (auto j_alg : _configs["camera_all"])
          {
            if (algName == j_alg["algorithm"])
          ...
          cv::Mat img,grayM1, grayM2, dstImage;
          img=image.clone();
          int checkValue = cam_config["config"]["checkValue"];
          if (checkValue != m3[rtsp])
          {
              m2[rtsp] = img.clone();
              m3[rtsp] = checkValue;
              cv::imwrite(algName+"/"+sanitizeRtspUrl(rtsp)+".jpg", m2[rtsp]);
          }
          if(m2[rtsp].empty()){
              m2[rtsp] = img.clone();
              cv::imwrite(algName+"/"+sanitizeRtspUrl(rtsp)+".jpg", m2[rtsp]);
          ...
          cv::cvtColor(m2[rtsp], grayM2, cv::COLOR_BGR2GRAY);
          ...
          boxes = yolo_model.infer(dstImage);
        }
        else if(algName=="cv_asset_deficiency_detection" ){
          json cam_config;
          for (auto j_alg : _configs["camera_all"])
          {
            if (algName == j_alg["algorithm"])
        ...
        else if (algName == "smoking_detection")
...
public:
  void run()
  {
    ...
    vector<string> alg_filter = {"crowd_density_statistics", "cv_ship_count_statistics", "cv_personnel_flow_statistics", "pointer_instrument_detection2",
      ...
      "cv_traffic_flow", "face_detection", "license_plate_detection", "cv_large_car_detection", "cv_parking_license_plate_detection", "cv_road_parking_license_plate_detection", "cv_vehicle_departure_detection", "cv_livestock_license_plates_detection"};
    int cds1_status = 0;
    std::shared_ptr<CDS_SERVER> cds1;
    int cds_boat_status = 0;
    std::shared_ptr<CDS_SERVER> cds_boat;
    int cds_mechanical_status = 0;
    std::shared_ptr<HttpMechanicalImage> cds_mechanical; // HttpMechanicalImage getM;//机械表读数默认不开
      ...
      try
      {
        ...
        if (auto res = httplib::Client(ip_port.c_str()).Post("/api/camera/all"))
        {
          ...
          for (auto it : j)
          {
            if (std::find(alg_filter.begin(), alg_filter.end(), it["algorithm"]) == alg_filter.end())
            {
              if (sTypeName != "")
              {
                if (it["algorithm"] == sTypeName){
                  ...
                  for (const auto &cam : it["cams"])
                  {
                    const std::string rtsp = cam.value("rtsp", "");
                    if (!rtsp.empty() and rtsp != "__placeholder__") //机械表虚拟相机
                    {
            ...
            if (it["algorithm"] == "pointer_instrument_detection2")
            {
              if (cds_mechanical_status == 0)
              {
                cds_mechanical = std::make_shared<HttpMechanicalImage>();
                cds_mechanical_status = 1;
                sleep(1);
              }
            }
          }
        ...
        if (auto res = httplib::Client(ip_port.c_str()).Get("/api/camera/preview/rtsp"))
        {
          ...
          for (auto j_rtsp : j)
          {
            if (j_rtsp["rtsp"] != "__placeholder__"){
