...
#define LOG_DEBUG(msg) std::cout << "[DEBUG] " << __FILE__ << ":" << __LINE__ << " (" << __func__ << "): " << msg << std::endl;
...
public:
    ...
    void makeSendMsg(cv::Mat &frame, int &alarm, json &jresult, json &jwarn, json roi)
    {
        ...
        if (alarm)
        {
			...
			// 添加报警消息 
			if (b_addVideo && AlarmVideoInfo::switchAlarm){
...
//									LOG_DEBUG("current time = " + std::to_string(t) + "/ new person time = " + std::to_string(it->second) + "/ old person time" + std::to_string(stamp));
	...
	void LicensePlateProcess(outDetectData postmsg, json cam_config, int cam_config_open)
    {
		int alarm = 0;
		cv::Mat frame = postmsg.img;
        json jresult;
        json jwarn;
		std::vector<BoxInfo> generate_boxes = postmsg.boxes;
        ...
        if (statusInTimes(cam_config["config"]["work_time"]))
		{
//            if ((int)oldtime < 0) // 初始化
//                oldtime = cam_config["config"]["time"].get<float>();
//            if (!(abs(oldtime - (float)cam_config["config"]["time"]) < 0.0000001))
//            {
//                std::cout << "--[INFO] time is reset:" << alg_name << rtsp_path << std::endl;
//                oldtime = cam_config["config"]["time"].get<float>();
//                time_start_dict.clear(); // 陌生人的持续时间
//            }
			...
			for (auto item : frameTrackingResult)
			{
				...
				std::string id_label = to_string(item.id) + "_" + to_string(item.label);
				it = time_start_dict.find(id_label);
				if (it == time_start_dict.end())
				{
					time_start_dict[id_label] = t; // 防止一开始就报警
				...
				std::string carCode;
				if (alg_name == "license_plate_tracking" || alg_name == "license_plate_detection")
				{
					if (alarm == 1)
					{
		
						cv::Mat img = postmsg.img;
						...
						premsg.inBox = box;
//						if (queue_lpr_pre.Push(premsg) != 0)
//						{
//							usleep(1000);
//						}
//						// 要同步，等算完10s
...
//						if (aa->Pop(postmsg, 10 * 1000) != 0)
//						{
//							usleep(1000);
//							continue;
//						}
						CarInfer::instance().plateInfer(premsg, postmsg);
				...
				int jx1 = (float)item.box.x * (float)1280.f / ((float)frame.cols);
				int jy1 = (float)item.box.y * ((float)previewH / (float)frame.rows);
				int jx2 = (float)(item.box.x + item.box.width) * ((float)1280.f / (float)frame.cols);
				int jy2 = (float)(item.box.y + item.box.height) * ((float)previewH / (float)frame.rows);
				jboxone.emplace_back(jx1);
				jboxone.emplace_back(jy1);
				jboxone.emplace_back(jx2);
				jboxone.emplace_back(jy2);
				json jboxone2;
