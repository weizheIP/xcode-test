...
public:
  ...
  void run()
  {
    ...
    else if (algName == "face_detection" || algName == "personnel_duty_detection" || algName == "anti_theft_detection" || algName == "stranger_use_terminal_detection")
    {
      facetool.init("");
    }
    else
    ...
    while (1)
    {
      ...
      for (const auto &pair : _rtsp_image5)
      {
        ...
        if (algName == "face_detection" || algName == "personnel_duty_detection" || algName == "anti_theft_detection" || algName == "stranger_use_terminal_detection")
        {
          json cam_config;
          for (auto j_alg : _configs["camera_all"])
          {
...
//		  draw_face_keypoints(face_result);
...
public:
  void run()
  {
    ...
    vector<string> alg_filter = {"crowd_density_statistics", "cv_ship_count_statistics", "cv_personnel_flow_statistics", "pointer_instrument_detection2",
      ...
      "cv_traffic_flow",  "license_plate_detection", "cv_large_car_detection", "cv_parking_license_plate_detection", "cv_road_parking_license_plate_detection", "cv_vehicle_departure_detection", "cv_livestock_license_plates_detection"};
    // int cds1_status = 0;
