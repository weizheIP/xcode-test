...
bool getstypearr(std::string strs,std::string sign=","){
    ...
    // 临时加了"222222"
    // const char *suffix = "222222";
    // size_t buffer_size = strlen(data) + strlen(suffix) + 1;
    // char *buffer = new char[buffer_size];
    // snprintf(buffer, buffer_size, "%s%s", data, suffix);
    // MD5_Init(&ctx);
...
bool checklic(std::string sLicense_path)
{
    if(!access(sLicense_path.c_str(),NULL)){
        FILE* f = fopen(sLicense_path.c_str(),"rb");
        fseek(f,0,SEEK_END);
        long size_f = ftell(f);
