...
public:
    int status;
    websocket_syn(std::string url,std::string port):url_(url),port_(port){
        status = init();}
    ~websocket_syn(){
        delete resolver;
        try
    ...
    int init(){
        ...
        auto const results = resolver->resolve(url_, port_);
