...
// #else
// 初始化更改
int anchor0[6] = {4, 5, 8, 10, 13, 16};
int anchor1[6] = {23, 29, 43, 55, 73, 105};
int anchor2[6] = {146, 217, 231, 300, 335, 433};
// #endif
...
{
  #ifdef USE_CROSS
  int OBJ_CLASS_NUM = 1; // 0:4 框，5是prob，5：13是landmark，13是类别
  #else
  int OBJ_CLASS_NUM = 2; // 0:4 框，5是prob，5：13是landmark，13是类别
  #endif
  int PROP_BOX_SIZE =5+OBJ_CLASS_NUM+8;
    ...
    for (int i = 0; i < grid_h; i++) {
      for (int j = 0; j < grid_w; j++) {
        ...
        if (box_confidence >= thres) {
          ...
          float maxClassProbs = in_ptr[13 * grid_len];
          int    maxClassId    = 0;
          for (int k = 1; k < OBJ_CLASS_NUM; ++k) {
            float prob = in_ptr[(13 + k) * grid_len];
          ...
          if (maxClassProbs>thres){
            ...
            for (int tk = 0; tk < 4; ++tk) {
                float landm1_x =in_ptr[(5 + 2*tk) * grid_len]* (float)anchor[a * 2] + j* (float)stride;
                float landm1_y =in_ptr[(6 + 2*tk) * grid_len]* (float)anchor[a * 2 +1] + i* (float)stride;
                temp_proto.push_back(landm1_x);
                temp_proto.push_back(landm1_y);
            }
                 ...
                 std::vector<float>& qnt_scales, detect_result_group_t* group)
{
  ...
  for (int i = 0; i < validCount; ++i) {
    ...
    group->results[last_count].landmark   = mask_feat[n];
