...
// int post_process(float* input0, float* input1, float* input2, float* maskData, int model_in_h, int model_in_w, float conf_threshold,
int post_process(float* input0, float* input1, float* input2,float* input3, float* maskData, int model_in_h, int model_in_w, float conf_threshold,
                 ...
                 std::vector<float>& qnt_scales, detect_result_group_t* group)
{
  ...
  for (int i = 0; i < validCount; ++i) {
    ...
    // rect_160s.push_back(cv::Rect(int(x1/4), int(y1/4), int(x2/4 - x1/4), int(y2/4 - y1/4)));
    ...
    x1=(int)(clamp(x1, 0, model_in_w) );
    y1=(int)(clamp(y1, 0, model_in_h) );
    x2=(int)(clamp(x2, 0, model_in_w) );
    y2=(int)(clamp(y2, 0, model_in_h) );    
    rect_160s.push_back(cv::Rect(int(x1/4), int(y1/4), int(x2/4 - x1/4), int(y2/4 - y1/4)));
