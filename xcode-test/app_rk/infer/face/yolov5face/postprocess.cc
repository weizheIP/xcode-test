...
{
  int OBJ_CLASS_NUM=1; // 0:4 框，5是prob，5：15是landmark，15是类别
  int PROP_BOX_SIZE =5+OBJ_CLASS_NUM+10;
  ...
  for (int a = 0; a < 3; a++) {
    for (int i = 0; i < grid_h; i++) {
      for (int j = 0; j < grid_w; j++) {
        ...
        // std::cout <<"validCount1----"<< box_confidence<<"--"<<thres<<"--"<<(box_confidence-thres)<< std::endl; 
        // if (box_confidence>=thres){ //10.3967>0.405465为什么不往下走，浮点精度误差问题，和是否debug模式没关系————没有过滤会很慢
        // if (1) {
