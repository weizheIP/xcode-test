...
MPP_RET MppEncoder::encode(const void* img, int img_len, char* dst, int *length)
{
	...
	*length = 0;
	...
	if (0 == (countIdx_ % args_.fps_out_num)) {
		countIdx_ = 0;
		int len = 0;
...
MPP_RET MppEncoder::test_mpp_enc_cfg_setup(MpiEncData *p) {
    ...
    case MPP_VIDEO_CodingMJPEG : {
		...
		mpp_enc_cfg_set_s32(cfg, "jpeg:q_factor", 50);
        mpp_enc_cfg_set_s32(cfg, "jpeg:qf_max", 50);
        mpp_enc_cfg_set_s32(cfg, "jpeg:qf_min", 50);
    } break;
    default : {
    } break;
    }
...
void MppEncoder::setUp(int width, int height, int fps, MppCodingType encodeType = MPP_VIDEO_CodingAVC, MppFrameFormat frameFormat = MPP_FMT_BGR888) {
	...
	args_.format = frameFormat;

	//设置264编码格式， AVC
	args_.type = encodeType;
